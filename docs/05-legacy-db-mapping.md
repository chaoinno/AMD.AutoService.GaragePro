...
| B | ~~อู่ที่จะใช้ระบบใหม่ ทำงาน service ด้วยหรือทำแต่งานเคลม~~ **ตอบแล้ว (2026-09-10)** | `Branch.BranchGroupId = 7` คือกลุ่ม "Service" — ยืนยันโดยผู้ใช้ enforce แล้วที่ `LegacyUserReader.GetAccessibleBranchesAsync`/`BuildAccessibleBranchesSql` (login และเปิดกะกรองเฉพาะสาขากลุ่มนี้ แม้ผู้ดูแลระบบก็เห็นเฉพาะกลุ่มนี้) — ยังไม่ได้ตรวจว่ามีค่า `BranchGroupId` อื่นที่ควรนับเป็น "Service" ด้วยหรือไม่ (เช่น สาขาผสม service+เคลม) ถ้าเจอเพิ่มต้องยืนยันแยก |
