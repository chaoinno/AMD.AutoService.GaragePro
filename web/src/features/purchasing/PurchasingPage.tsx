...
import { CheckCircle2, CircleOff, ClipboardList, Clock3, FileSpreadsheet, Plus, Printer, RefreshCw, Search, Send, ShoppingCart, Trash2 } from 'lucide-react'
...
function PurchaseEditor({ kind, doc, onClose, onSaved }: { kind: PurchaseKind; doc?: Purchase; onClose: () => void; onSaved: (doc: Purchase) => void }) {
  ...
  const blankRow = () => ({ rowId: crypto.randomUUID(), catalogItemId: '', code: '', name: '', unit: '', quantity: '1', unitCost: '0' })
  const [lines, setLines] = useState(() => doc?.lines.length ? doc.lines.map(x => ({ rowId: crypto.randomUUID(), catalogItemId: x.catalogItemId, code: x.code, name: x.name, unit: x.unit, quantity: String(x.quantity), unitCost: String(x.unitCost) })) : [blankRow()])
  ...
  const submit = () => {
    ...
    if (lines.some(x => !x.catalogItemId)) { setError('กรุณาเลือกสินค้าจากรายการค้นหาให้ครบทุกแถว หรือลบแถวที่ไม่ใช้'); return }
  ...
  return <ConfirmModal open title={doc ? `แก้ไข ${doc.number}` : `สร้าง${kind === 'PR' ? 'ใบขอซื้อ (PR)' : 'ใบสั่งซื้อ (PO)'}`} description="ออกเลขเอกสารอัตโนมัติเมื่อบันทึก · จำนวนสินค้าเป็นจำนวนเต็ม" size="xlarge" onClose={() => { if (!save.isPending) onClose() }} footer={<><Button variant="ghost" onClick={onClose} disabled={save.isPending}>ยกเลิก</Button><Button form="purchase-editor" type="submit" disabled={save.isPending}>{save.isPending ? 'กำลังบันทึก…' : 'บันทึกฉบับร่าง'}</Button></>}>
    <form id="purchase-editor" className="management-form" onSubmit={e => { e.preventDefault(); submit() }}><fieldset disabled={save.isPending} className="purchase-fieldset"><div className="form-grid">
      ...
      {!lockedLines && <div className="purchase-list-actions"><label htmlFor="purchase-row-count">จำนวนแถวที่เพิ่ม</label><Input id="purchase-row-count" type="number" min="1" max="50" step="1" value={rowsToAdd} onChange={e => setRowsToAdd(e.target.value)} style={{ width: 90 }} /><Button type="button" variant="outline" onClick={() => { const count = Number(rowsToAdd); if (!Number.isInteger(count) || count < 1 || count > 50) { setError('เพิ่มได้ครั้งละ 1–50 แถว'); return } setLines(old => [...old, ...Array.from({ length: count }, blankRow)]); setError('') }}><Plus aria-hidden="true" /> เพิ่มแถว</Button><span className="section-help">ค้นหาสินค้าในตาราง · ใช้ลูกศรและ Enter เพื่อเลือก</span></div>}
      {lockedLines && <p className="section-help">รายการและจำนวนอ้างอิง PR ที่อนุมัติแล้ว สามารถแก้ไขราคาซื้อได้</p>}
      <div className="purchase-table-scroll"><table className="master-table purchase-lines purchase-entry-lines"><thead><tr><th>ลำดับ</th><th>สินค้า</th><th>หน่วย</th><th>จำนวน</th><th>ราคา/หน่วย</th><th>รวม</th><th /></tr></thead><tbody>{lines.map((line, i) => <tr key={line.rowId}><td className="purchase-entry-sequence">{i + 1}</td><td>{lockedLines ? <><strong>{line.name}</strong><small className="purchase-sub">{line.code}</small></> : <><PurchaseItemPicker rowId={line.rowId} label={line.catalogItemId ? `${line.code} · ${line.name}` : ''} excluded={lines.filter((_, n) => n !== i).map(x => x.catalogItemId)} onSelect={item => setLines(old => old.map(row => row.rowId === line.rowId ? { ...row, catalogItemId: item.id, code: item.code, name: item.name, unit: item.unit, unitCost: String(item.cost ?? 0) } : row))} /></>}</td><td className="purchase-entry-unit">{line.unit || '—'}</td><td><Input aria-label={`จำนวน ${line.name}`} required type="number" min="1" max="1000000" step="1" readOnly={lockedLines} value={line.quantity} onChange={e => setLines(old => old.map((x, n) => n === i ? { ...x, quantity: e.target.value } : x))} /></td><td><Input aria-label={`ราคา ${line.name}`} required type="number" min="0" max="100000000" step="0.01" value={line.unitCost} onChange={e => setLines(old => old.map((x, n) => n === i ? { ...x, unitCost: e.target.value } : x))} /></td><td className="money">{money(Number(line.quantity) * Number(line.unitCost))}</td><td>{!lockedLines && <Button variant="ghost" size="icon" aria-label={`ลบ ${line.name}`} onClick={() => setLines(old => old.filter((_, n) => n !== i))}><Trash2 /></Button>}</td></tr>)}</tbody></table></div>
