...
export function SupplierPage() {
  ...
  return <AppShell title="ซัพพลายเออร์">
    ...
    <QueryState query={{ ...query, data: suppliers }} loadingTitle="กำลังโหลดซัพพลายเออร์" emptyTitle="ยังไม่มีซัพพลายเออร์" emptyReason="เพิ่มผู้จำหน่ายรายแรก หรือเปลี่ยนตัวกรองสถานะเพื่อดูรายการอื่น" onRetry={() => void query.refetch()}>
      <Card className="management-table-card"><ManagementTable data={suppliers} columns={[
        { id: 'supplier', header: 'ซัพพลายเออร์', value: (item) => item.name, size: 250, render: (item) => <><div className="master-name"><strong>{item.name}</strong><small>{item.code}</small></div></> },
        { id: 'contact', header: 'ผู้ติดต่อ', value: (item) => [item.contactName, item.phone, item.email].filter(Boolean).join(' '), size: 260, render: (item) => <><div className="master-contact">{item.contactName ? <span><UserRound /> {item.contactName}</span> : null}{item.phone ? <span><Phone /> {item.phone}</span> : null}{item.email ? <span><Mail /> {item.email}</span> : null}{!item.contactName && !item.phone && !item.email ? <span className="muted">ไม่ระบุผู้ติดต่อ</span> : null}</div></> },
        { id: 'terms', header: 'เงื่อนไขชำระเงิน', value: (item) => item.paymentTerms, render: (item) => <>{item.paymentTerms || <span className="muted">ไม่ระบุ</span>}</> },
