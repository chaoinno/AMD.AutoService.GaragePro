...
export function VehiclePage() {
  ...
  const columns = useMemo<ColumnDef<VehicleSummary, unknown>[]>(() => [
    { id: 'image', header: 'รูป', size: 76, cell: ({ row }) => row.original.imageUrl ? <ProtectedImage path={row.original.imageUrl} alt={`รถทะเบียน ${row.original.registration}`} /> : <span className="vehicle-placeholder"><CarFront /></span> },
    { id: 'registration', header: 'ทะเบียน', size: 170, cell: ({ row }) => <div className="two-line-cell"><strong>{row.original.registration}</strong><span>{row.original.provinceName || 'ไม่ระบุจังหวัด'}</span></div> },
    { id: 'model', header: 'รถ', size: 245, cell: ({ row }) => <div className="two-line-cell"><strong>{row.original.brandName || 'ไม่ระบุยี่ห้อ'} {row.original.modelName || ''}</strong><span>{row.original.nickname || 'ไม่ระบุโฉม'} · {row.original.year || 'ไม่ระบุปี'}</span></div> },
    { id: 'owner', header: 'ลูกค้า', size: 230, cell: ({ row }) => <div className="two-line-cell"><strong>{row.original.ownerName || 'ไม่ระบุเจ้าของ'}</strong><span>{row.original.ownerPhone || 'ไม่ระบุเบอร์โทร'}</span></div> },
    { id: 'type', header: 'ชนิด / สี', size: 165, cell: ({ row }) => <div className="two-line-cell"><strong>{row.original.carTypeName || 'ไม่ระบุชนิด'}</strong><span>{row.original.primaryColorName || 'ไม่ระบุสี'}</span></div> },
    { id: 'status', header: 'สถานะ', size: 105, cell: ({ row }) => row.original.isDeleted ? <Badge variant="outline">ลบแล้ว</Badge> : <Badge className="active-badge">ใช้งาน</Badge> },
  ...
  else content = <Card className="management-table-card"><DataTable data={query.data.items} columns={columns} onRowClick={(row) => setFormId(row.id)} getRowLabel={(row) => `เปิดข้อมูลรถ ${row.registration}`} /><Pagination page={query.data.page} totalPages={query.data.totalPages} totalItems={query.data.totalItems} onPageChange={(page) => setFilters((f) => ({ ...f, page }))} /></Card>
...
function VehicleFormDialog({ open, vehicleId, onClose }: { open: boolean; vehicleId: number | null; onClose: () => void }) {
  ...
  return <ConfirmModal open={open} title={vehicleId ? 'แก้ไขข้อมูลรถ' : 'เพิ่มรถลูกค้า'} description="รถที่บันทึกจะแสดงในรายการทันทีโดยไม่ต้องรอมีงานซ่อม" onClose={close} size="large" footer={<><Button variant="ghost" onClick={close}>ยกเลิก</Button><Button type="submit" form="vehicle-form" disabled={save.isPending || details.isPending}>{save.isPending ? 'กำลังบันทึก…' : 'บันทึกข้อมูลรถ'}</Button></>}>
    {details.isPending ? <SkeletonRows count={5} /> : <form id="vehicle-form" className="management-form" onSubmit={submit}>
