...
function CustomerFormDialog({ open, customerId, onClose }: { open: boolean; customerId: number | null; onClose: () => void }) {
  ...
  return <>
    <ConfirmModal open={open} title={customerId ? 'แก้ไขข้อมูลลูกค้า' : 'เพิ่มลูกค้า'} description="ข้อมูลที่บันทึกจะใช้ร่วมกับระบบ GaragePro เดิม" onClose={close} size="large" footer={<><Button variant="ghost" onClick={close}>ยกเลิก</Button><Button type="submit" form="customer-form" disabled={save.isPending || details.isPending}>{save.isPending ? 'กำลังบันทึก…' : 'บันทึกข้อมูล'}</Button></>}>
      {details.isPending ? <SkeletonRows count={4} /> : <form id="customer-form" className="management-form" onSubmit={submit}>
