...
export function StaffPage() {
  ...
  const columns = useMemo<ColumnDef<StaffSummary, unknown>[]>(() => [
    { id: 'staff', accessorFn: (x) => x.fullName, header: 'พนักงาน', size: 300, cell: ({ row }) => <div className="staff-name-cell"><span className="staff-initial">{row.original.pictureUrl ? <StaffPhoto key={`${row.original.id}:${row.original.lastUpdated}:${row.original.pictureUrl}`} staffId={row.original.id} name={row.original.fullName} /> : <span title="ยังไม่มีรูปพนักงาน">{initials(row.original.fullName)}</span>}</span><span><strong>{row.original.fullName}</strong><small>{row.original.code}{row.original.isAdministrator ? ' · ผู้ดูแลระบบ' : ''}</small></span></div> },
    { id: 'organization', accessorFn: (x) => [x.departmentName, x.sectorName, x.positionName].filter(Boolean).join(' '), header: 'ฝ่าย / แผนก / ตำแหน่ง', size: 280, cell: ({ row }) => <div className="two-line-cell"><strong>{row.original.departmentName || 'ไม่ระบุฝ่าย'} · {row.original.sectorName || 'ไม่ระบุแผนก'}</strong><span>{row.original.positionName || 'ไม่ระบุตำแหน่ง'}</span></div> },
    { id: 'contact', accessorFn: (x) => [x.phoneNumber1, x.email].filter(Boolean).join(' '), header: 'การติดต่อ', size: 210, cell: ({ row }) => <div className="two-line-cell"><strong>{row.original.phoneNumber1 || 'ไม่ระบุเบอร์'}</strong><span>{row.original.email || 'ไม่ระบุอีเมล'}</span></div> },
    { id: 'status', accessorFn: (x) => x.isActive ? 'ใช้งาน' : 'ปิดใช้งาน', header: 'สถานะ', size: 130, cell: ({ row }) => row.original.isActive ? <Badge className="active-badge"><CheckCircle2 /> ใช้งาน</Badge> : <Badge variant="outline"><Ban /> ปิดใช้งาน</Badge> },
  ...
  else content = <Card className="management-table-card"><DataTable sortable sortScope="page" data={query.data.items} columns={columns} onRowClick={(x) => setFormId(x.id)} getRowLabel={(x) => `เปิดข้อมูลพนักงาน ${x.fullName}`} /><Pagination page={query.data.page} totalPages={query.data.totalPages} totalItems={query.data.totalItems} onPageChange={(page) => setFilters((f) => ({ ...f, page }))} /></Card>
...
function StaffFormDialog({ open, staffId, onClose }: { open: boolean; staffId: number | null; onClose: () => void }) {
  ...
  const submit = handleSubmit((v) => save.mutate({ input: {
    branchId, firstName: v.firstName, lastName: v.lastName, genderId: Number(v.genderId), idCard: v.idCard || undefined,
  ...
  const lockedReason = !isAdmin && staffId ? 'เฉพาะ Admin เท่านั้นที่แก้ไขแผนก และตำแหน่งได้' : undefined
...
function StaffPhoto({ staffId, name }: { staffId: number; name?: string }) {
  const [url, setUrl] = useState<string | null>(null)
  useEffect(() => {
    let active = true
    let objectUrl: string | null = null
    setUrl(null)
    // [SECURITY] Fetch through the authenticated, branch-scoped API, never a public image URL.
    void getStaffImage(staffId).then((blob) => {
      if (!active) return
  ...
  return url
    ? <img src={url} alt={name ? `รูปพนักงาน ${name}` : 'รูปพนักงานปัจจุบัน'} decoding="async" onError={() => setUrl(null)} />
    : name ? <span title="ไม่มีรูปหรือไม่สามารถโหลดรูปพนักงานได้">{initials(name)}</span> : <UserRound />
}
